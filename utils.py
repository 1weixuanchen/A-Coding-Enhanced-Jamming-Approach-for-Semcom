...
class FFHQDataset(Dataset):
    def __init__(self, data_root, transform=None, start_idx=0, end_idx=60000):

        self.data_root = data_root
        self.transform = transform
        self.image_files = [f"{i:05d}.png" for i in range(start_idx, end_idx)]
...
class MNISTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_idx = self.start_idx + idx

        image_name = f"{image_idx}.png"
...
def data_loader_ffhq(data_root, batch_size, transform_train, transform_test):
    transform_train = transforms.Compose(transform_train)
    transform_test = transforms.Compose(transform_test)

    train_dataset = FFHQDataset(data_root=data_root, transform=transform_train, start_idx=0, end_idx=50000)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

    test_dataset = FFHQDataset(data_root=data_root, transform=transform_test, start_idx=50000, end_idx=60000)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)

    return train_loader, test_loader


def data_loader_mnist(data_root, batch_size, transform_train, transform_test):
    transform_train = transforms.Compose(transform_train)
    transform_test = transforms.Compose(transform_test)

    train_dataset = MNISTDataset(data_root=data_root, transform=transform_train, start_idx=0, end_idx=50000)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

    test_dataset = MNISTDataset(data_root=data_root, transform=transform_test, start_idx=50000, end_idx=60000)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)
...
def data_loader(data_root, batch_size, transform_train, transform_test):
    ...
    cifar100_training = torchvision.datasets.CIFAR10(
        root=data_root,
        train=True,
        transform=None,
        download=True,
    )
    train_set = Cifar10(cifar100_training, transform_train)
    train_loader = DataLoader(train_set, batch_size, True)

    cifar100_testing = torchvision.datasets.CIFAR10(
        root=data_root,
        train=False,
        transform=None,
        download=True,
    )
    test_set = Cifar10(cifar100_testing, transform_test)
