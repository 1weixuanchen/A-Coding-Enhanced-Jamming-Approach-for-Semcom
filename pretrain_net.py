...
from basic_module import awgn, Decoder_Recon, normalize, Encoder
...
class Analog(nn.Module):
    def __init__(self, config, device):
        ...
        if config.net == 'analog_bad':
            self.class_decoder = Decoder_Recon(self.Half_width, self.layer_width, 20)
        elif config.net == 'analog_good':
            self.class_decoder = Decoder_Recon(self.Half_width, self.layer_width, 100)
